...
Each test case is independent, so it is dispatched to a process pool and the
results are aggregated in the parent process exactly as in the serial version.
"""
...
def process_test_case(test_case, num_sim_replications, ci_level):
    ...
    # Compute mean and CI for each metric from sim replications
    ...
    for metric in METRICS_LIST:
        ...
        row[f'ci_lower_{metric}'] = ci_lower[metric]
        row[f'ci_upper_{metric}'] = ci_upper[metric]
        row[f'ci_cover_{metric}'] = ci_coverage[metric]

    # Compact per-case log (prints from workers may interleave)
    print(f"[case {case_id}] done: "
          f"λ={test_case['arrival_rate']:.1f}, μ={test_case['service_rate']:.1f}, "
          f"servers={test_case['num_servers']}, warm%={test_case['warm_percent']:.1f}, "
          f"spawn={test_case['spawn_time']} | "
...
def main():
    ...
    print("Starting Markov Model vs Simulator Comparison (multiprocessing)")
