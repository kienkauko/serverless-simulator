#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Comparison script between Markov Model and Simulator
Runs both systems with the same input parameters and compares their results
"""

import numpy as np
import pandas as pd
import math
import random
import simpy
import sys
import os
from datetime import datetime
import warnings
warnings.filterwarnings('ignore')

# Import Markov models - will select based on user input
from model_3D import MarkovModel as MarkovModel3D
from model_4D import MarkovModel as MarkovModel4D

# Import simulator components
from Server import Server
from Request import Request
from Container import Container
from System import System

def generate_test_cases(num_cases=50, model_type='3D'):
    """Generate test cases by picking random values within predefined ranges."""
    # Define parameter ranges
    ranges = {
        'arrival_rate': (1.0, 100.0),   # λ between 10 and 30
        'service_rate': (0.25, 4.0),    # μ between 0.25 and 4
        'num_servers': (1, 20),         # servers between 1 and 20 (integers)
        'timeout': (1.0, 50.0),         # timeout between 0.5 and 10.0
        'spawn_time': (0.01, 1.0)       # spawn time between 0.5 and 10.0
    }
    
    # Add 4D specific parameter ranges
    if model_type == '4D':
        ranges.update({
            'gamma': (0.01, 1.0),          # gamma between 0.5 and 10.0
            'spawn_model_rate': (0.5, 10.0), # spawn model rate between 0.5 and 10.0
            'power_max': (100.0, 200.0),     # peak power between 100 and 200
            'power_min_scale': (0.2, 0.6),   # power scale between 0.3 and 0.7
            'num_servers': (1, 5)            # For 4D, limit to max 5 servers
        })

    # Define fixed resource requirements
    cpu_warm = 1
    ram_warm = 20
    cpu_warm_model = 1
    ram_warm_model = 30
    cpu_demand = 50
    ram_demand = 40

    test_cases = []
    for case_id in range(num_cases):
        test_case = {
            'case_id': case_id,
            'arrival_rate': random.uniform(*ranges['arrival_rate']),
            'service_rate': random.uniform(*ranges['service_rate']),
            'num_servers': random.randint(ranges['num_servers'][0], ranges['num_servers'][1]),
            'timeout': random.uniform(*ranges['timeout']),
            'spawn_time': random.uniform(*ranges['spawn_time']),
            # Add resource consumption to all test cases
            'cpu_warm': cpu_warm,
            'ram_warm': ram_warm,
            'cpu_warm_model': cpu_warm_model,
            'ram_warm_model': ram_warm_model,
            'cpu_demand': cpu_demand,
            'ram_demand': ram_demand
        }
        
        # Add 4D specific parameters
        if model_type == '4D':
            test_case.update({
                'gamma': random.uniform(*ranges['gamma']),
                'spawn_model_rate': random.uniform(*ranges['spawn_model_rate']),
                'power_max': random.uniform(*ranges['power_max']),
                'power_min_scale': random.uniform(*ranges['power_min_scale'])
            })

        test_cases.append(test_case)

    return test_cases

def convert_to_markov_config(test_case, model_type='3D'):
    """Convert test case to Markov model configuration"""
       
    # Get resource values from the test case
    cpu_warm = test_case['cpu_warm']
    ram_warm = test_case['ram_warm']
    cpu_warm_model = test_case['cpu_warm_model']
    ram_warm_model = test_case['ram_warm_model']
    cpu_demand = test_case['cpu_demand']
    ram_demand = test_case['ram_demand']
    
    # Calculate queue sizes based on simulator logic
    max_resource = max(cpu_demand, ram_demand)
    total_containers = math.floor(100 / max_resource) * test_case['num_servers']
    
    markov_config = {
        "arrival_rate": test_case['arrival_rate'],
        "service_rate": test_case['service_rate'],
        "spawn_rate": 1.0 / test_case['spawn_time'],
        "max_queue": total_containers,
        "theta": 1.0/ test_case['timeout'],  # Utilization factor
        "ram_warm": ram_warm,
        "cpu_warm": cpu_warm,
        "ram_demand": ram_demand,
        "cpu_demand": cpu_demand,
        "power_max": test_case['power_max'],
        "power_min_scale": test_case['power_min_scale']
    }
    
    # Add 4D specific parameters
    if model_type == '4D':
        markov_config.update({
            "ram_warm_model": ram_warm_model,
            "cpu_warm_model": cpu_warm_model,
            "spawn_model_rate": test_case['spawn_model_rate'],
            "gamma": test_case['gamma']
        })
    
    return markov_config

def convert_to_simulator_config(test_case, model_type='3D'):
    """Convert test case to simulator configuration"""
    
    sim_config = {
        # System Parameters
        "system": {
            "num_servers": test_case['num_servers'],
            "sim_time": 6000,  # Longer simulation for stability
            "distribution": "deterministic",  # Use exponential distribution for request arrivals
            "verbose": False
        },
        
        # Server Parameters
        "server": {
            "cpu_capacity": 100.0,
            "ram_capacity": 100.0,
            "power_max": test_case['power_max'],
            "power_min_scale": test_case['power_min_scale']
        },
        
        # Request Parameters
        "request": {
            "arrival_rate": test_case['arrival_rate'],
            "service_rate": test_case['service_rate'],
            "warm_cpu": test_case['cpu_warm'],
            "warm_ram": test_case['ram_warm'],
            "warm_cpu_model": test_case['cpu_warm_model'],
            "warm_ram_model": test_case['ram_warm_model'],
            "cpu_demand": test_case['cpu_demand'],
            "ram_demand": test_case['ram_demand'],
        },
        
        # Container Parameters
        "container": {
            "spawn_time": test_case['spawn_time'],
            "idle_cpu_timeout": test_case['timeout'],  # Use the timeout from the test case
            "idle_model_timeout": 0.0001,  # Small value to avoid immediate removal
            "load_request_time": 1/10000,
            "load_model_time": 0.0001,
        },
    }
    
    # Add 4D specific parameters
    if model_type == '4D':
        sim_config["container"].update({
            "load_model_time": 1.0 / test_case['spawn_model_rate'],
            "idle_model_timeout": 1.0 / test_case['gamma']
        })
    
    return sim_config

def run_markov_model(markov_config, model_type='3D'):
    """Run the Markov model and extract metrics"""
    try:
        if model_type == '3D':
            model = MarkovModel3D(markov_config, verbose=False)
        else:  # '4D'
            model = MarkovModel4D(markov_config, verbose=False)
            
        metrics = model.get_metrics()
        
        return {
            'blocking_probability': metrics['blocking_ratios'][0],
            'latency': metrics['latency'][0],
            'cpu_usage': metrics['cpu_usage_per_request'][0],
            'ram_usage': metrics['ram_usage_per_request'][0],
            'power_usage': metrics.get('power_usage_per_request', [0])[0] if model_type == '4D' else 0,
        }
    except Exception as e:
        print(f"Error in Markov model: {e}")
        return None

def run_simulator(sim_config):
    """Run the simulator and extract metrics"""
    try:        
        # Create simulation environment
        env = simpy.Environment()
        
        # Create system
        system = System(env, sim_config, distribution=sim_config["system"]["distribution"], 
                       verbose=sim_config["system"]["verbose"])
        
        # Add servers
        for i in range(sim_config["system"]["num_servers"]):
            server = Server(env, f"Server-{i}", sim_config["server"])
            system.add_server(server)
        
        env.process(system.request_generator())
        
        # Run simulation
        env.run(until=sim_config["system"]["sim_time"])
        
        # Get metrics from system
        metrics = system.get_metrics()
        
        return {
            'blocking_probability': metrics['blocking_ratios'][0],
            'latency': metrics['latency'][0],
            'cpu_usage': metrics['cpu_usage_per_request'][0],
            'ram_usage': metrics['ram_usage_per_request'][0],
            'power_usage': metrics['power_usage_per_request'][0] 
        }
        
    except Exception as e:
        print(f"Error in simulator: {e}")
        return None

def calculate_comparison_metrics(markov_results, sim_results):
    """Calculate MAPE, RMSE, and R-squared for comparison"""
    
    metrics = ['blocking_probability', 'latency', 'cpu_usage', 'ram_usage', 'power_usage']
    comparison_results = {}
    
    for metric in metrics:
        markov_vals = np.array([r[metric] for r in markov_results if r is not None])
        sim_vals = np.array([r[metric] for r in sim_results if r is not None])
        
        # Ensure we have the same number of valid results
        min_len = min(len(markov_vals), len(sim_vals))
        markov_vals = markov_vals[:min_len]
        sim_vals = sim_vals[:min_len]
        
        if len(markov_vals) == 0:
            comparison_results[metric] = {'MAPE': float('inf'), 'RMSE': float('inf'), 'R_squared': 0.0}
            continue
            
        # Calculate MAPE (Mean Absolute Percentage Error) - Markov vs Simulator reference
        # Special handling for very small values (like blocking probability)
        if metric == 'blocking_probability':
            # For blocking probability, use absolute error if values are very small
            threshold = 0.01  # 1% blocking probability threshold
            if np.mean(sim_vals) < threshold:
                # Use Mean Absolute Error instead of MAPE for very small values
                mape = np.mean(np.abs(markov_vals - sim_vals)) * 100  # Convert to percentage points
            else:
                mape = np.mean(np.abs((markov_vals - sim_vals) / np.maximum(sim_vals, 1e-10))) * 100
        else:
            mape = np.mean(np.abs((markov_vals - sim_vals) / np.maximum(sim_vals, 1e-10))) * 100
        
        # Calculate RMSE (Root Mean Square Error) - Markov vs Simulator reference
        rmse = np.sqrt(np.mean((markov_vals - sim_vals) ** 2))
        
        # Calculate Normalized RMSE (NRMSE) as percentage of mean
        nrmse = (rmse / np.mean(sim_vals)) * 100 if np.mean(sim_vals) > 0 else float('inf')
        
        # Calculate R-squared - Using Simulator as reference
        ss_res = np.sum((markov_vals - sim_vals) ** 2)
        ss_tot = np.sum((sim_vals - np.mean(sim_vals)) ** 2)
        r_squared = 1 - (ss_res / ss_tot) if ss_tot != 0 else 0.0
        
        comparison_results[metric] = {
            'MAPE': mape,
            'RMSE': rmse,
            'NRMSE': nrmse,  # Add this new metric
            'R_squared': r_squared
        }
    
    return comparison_results

def main():
    """Main comparison function"""
    print("Starting Markov Model vs Simulator Comparison")
    print("=" * 60)
    
    # Get model type choice from user
    while True:
        model_type = input("Choose model type (3D or 4D): ").strip()
        if model_type in ['3D', '4D']:
            break
        print("Invalid choice. Please enter '3D' or '4D'.")
    
    # Generate test cases
    # Get number of test cases from user
    while True:
        try:
            num_cases = int(input("Enter the number of test cases to generate: "))
            if num_cases > 0:
                break
            print("Please enter a positive integer.")
        except ValueError:
            print("Invalid input. Please enter a number.")
    
    test_cases = generate_test_cases(num_cases, model_type)
    
    print(f"Generated {len(test_cases)} test cases for model type {model_type}")
    
    # Store results
    markov_results = []
    sim_results = []
    detailed_results = []
    
    # Run comparisons
    for i, test_case in enumerate(test_cases):
        print(f"\nRunning test case {i+1}/{len(test_cases)}")
        print(f"Parameters: λ={test_case['arrival_rate']:.1f}, μ={test_case['service_rate']:.1f}, "
              f"servers={test_case['num_servers']}, timeout={test_case['timeout']:.1f}, "
              f"spawn_time={test_case['spawn_time']}")
        
        if model_type == '4D':
            print(f"  4D params: gamma={test_case['gamma']:.1f}, "
                  f"spawn_model_rate={test_case['spawn_model_rate']:.1f}, "
                  f"power_max={test_case['power_max']:.1f}, "
                  f"power_min_scale={test_case['power_min_scale']:.2f}")
        
        # Convert to respective configurations
        markov_config = convert_to_markov_config(test_case, model_type)
        sim_config = convert_to_simulator_config(test_case, model_type)
        
        # Run Markov model
        print("  Running Markov model...")
        markov_result = run_markov_model(markov_config, model_type)
        
        # Run simulator
        print("  Running simulator...")
        sim_result = run_simulator(sim_config)
        
        if markov_result is not None and sim_result is not None:
            markov_results.append(markov_result)
            sim_results.append(sim_result)
            
            # Store detailed results
            detailed_result = {
                'case_id': test_case['case_id'],
                'arrival_rate': test_case['arrival_rate'],
                'service_rate': test_case['service_rate'],
                'num_servers': test_case['num_servers'],
                'timeout': test_case['timeout'],
                'spawn_time': test_case['spawn_time'],
                'markov_blocking': markov_result['blocking_probability'],
                'sim_blocking': sim_result['blocking_probability'],
                'markov_latency': markov_result['latency'],
                'sim_latency': sim_result['latency'],
                'markov_cpu': markov_result['cpu_usage'],
                'sim_cpu': sim_result['cpu_usage'],
                'markov_ram': markov_result['ram_usage'],
                'sim_ram': sim_result['ram_usage']
            }
            
            # Add 4D specific metrics
            if model_type == '4D':
                detailed_result.update({
                    'gamma': test_case['gamma'],
                    'spawn_model_rate': test_case['spawn_model_rate'],
                    'power_max': test_case['power_max'],
                    'power_min_scale': test_case['power_min_scale'],
                    'markov_power': markov_result['power_usage'],
                    'sim_power': sim_result['power_usage']
                })
            
            detailed_results.append(detailed_result)
            
            print(f"  Markov: block={markov_result['blocking_probability']:.4f}, "
                  f"latency={markov_result['latency']:.4f}, "
                  f"cpu={markov_result['cpu_usage']:.2f}, ram={markov_result['ram_usage']:.2f}")
            
            print(f"  Sim:    block={sim_result['blocking_probability']:.4f}, "
                  f"latency={sim_result['latency']:.4f}, "
                  f"cpu={sim_result['cpu_usage']:.2f}, ram={sim_result['ram_usage']:.2f}")
            
            if model_type == '4D':
                print(f"  Power - Markov: {markov_result['power_usage']:.2f}, Sim: {sim_result['power_usage']:.2f}")
        else:
            print("  ERROR: One or both models failed!")

    # Calculate comparison metrics
    print(f"\n\nCalculating comparison metrics for {len(markov_results)} successful test cases...")
    comparison_metrics = calculate_comparison_metrics(markov_results, sim_results)
    
    # Print results
    print("\n" + "=" * 60)
    print("COMPARISON RESULTS")
    print("=" * 60)
    
    metrics_to_compare = ['blocking_probability', 'latency', 'cpu_usage', 'ram_usage']
    if model_type == '4D':
        metrics_to_compare.append('power_usage')
    
    print(metrics_to_compare)
    for metric in metrics_to_compare:
        print(f"\n{metric.upper().replace('_', ' ')}:")
        print(f"  MAPE:      {comparison_metrics[metric]['MAPE']:.2f}%")
        print(f"  RMSE:      {comparison_metrics[metric]['RMSE']:.6f}")
        print(f"  R-squared: {comparison_metrics[metric]['R_squared']:.4f}")
    
    # Save detailed results to CSV
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"comparison_results/markov_{model_type}_vs_sim_{timestamp}.csv"
    
    # Create directory if it doesn't exist
    os.makedirs("comparison_results", exist_ok=True)
    
    # Create a combined dataset with model identifier
    combined_results = []
    
    # Add Markov results
    for i, (test_case, markov_result) in enumerate(zip(test_cases[:len(markov_results)], markov_results)):
        result = {
            'case_id': test_case['case_id'],
            'model_type': f'Markov-{model_type}',
            'arrival_rate': test_case['arrival_rate'],
            'service_rate': test_case['service_rate'],
            'num_servers': test_case['num_servers'],
            'timeout': test_case['timeout'],
            'spawn_time': test_case['spawn_time'],
            'blocking_probability': markov_result['blocking_probability'],
            'latency': markov_result['latency'],
            'cpu_usage': markov_result['cpu_usage'],
            'ram_usage': markov_result['ram_usage']
        }
        
        if model_type == '4D':
            result.update({
                'gamma': test_case['gamma'],
                'spawn_model_rate': test_case['spawn_model_rate'],
                'power_max': test_case['power_max'],
                'power_min_scale': test_case['power_min_scale'],
                'power_usage': markov_result['power_usage']
            })
            
        combined_results.append(result)
    
    # Add Simulator results
    for i, (test_case, sim_result) in enumerate(zip(test_cases[:len(sim_results)], sim_results)):
        result = {
            'case_id': test_case['case_id'],
            'model_type': 'Simulator',
            'arrival_rate': test_case['arrival_rate'],
            'service_rate': test_case['service_rate'],
            'num_servers': test_case['num_servers'],
            'timeout': test_case['timeout'],
            'spawn_time': test_case['spawn_time'],
            'blocking_probability': sim_result['blocking_probability'],
            'latency': sim_result['latency'],
            'cpu_usage': sim_result['cpu_usage'],
            'ram_usage': sim_result['ram_usage']
        }
        
        if model_type == '4D':
            result.update({
                'gamma': test_case['gamma'],
                'spawn_model_rate': test_case['spawn_model_rate'],
                'power_max': test_case['power_max'],
                'power_min_scale': test_case['power_min_scale'],
                'power_usage': sim_result['power_usage']
            })
            
        combined_results.append(result)
    
    # Save combined results
    # df_combined = pd.DataFrame(combined_results)
    # df_combined.to_csv(filename, index=False)
    # print(f"\nCombined results saved to: {filename}")
    
    # Print summary statistics
    print(f"\nSUMMARY:")
    print(f"Total test cases: {len(test_cases)}")
    print(f"Successful comparisons: {len(markov_results)}")
    print(f"Success rate: {len(markov_results)/len(test_cases)*100:.1f}%")

if __name__ == "__main__":
    main()