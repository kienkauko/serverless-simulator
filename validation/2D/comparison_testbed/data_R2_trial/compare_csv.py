...
# Allow import from repo root (this script sits one level deeper than compare_csv.py)
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                '..', '..', '..', '..'))
from Markov.model_2D import MarkovModel

# ============================================================
# APPLICATION PROFILE — YOLO, from 2026_07_20_1624_lifecycle_result.txt
# (200 reps; RAM taken as % of node memory, not MB)
#
#   lifecycle stage    CPU %    RAM %    duration
#   null (baseline)     0.83    12.29    -           <- subtracted as baseline
#   null_to_warm_cpu    4.70     1.41     8.04 s     <- cold start / transit
#   warm_cpu           -0.04     2.47    -           <- warm idle
#   active              5.65     2.53     0.43 s     <- serving
# ============================================================
SERVICE_TIME_S     = 0.43       # active stage duration
SPAWN_TIME_S       = 8.04       # null_to_warm_cpu stage duration (cold start)
SPAWN_DISTRIBUTION = "exponential"

CPU_WARM           = 0.0        # measured -0.04 %, clamped to 0 (no CPU when warm-idle)
RAM_WARM           = 2.47
CPU_DEMAND         = 5.65
RAM_DEMAND         = 2.53
CPU_TRANSIT        = 4.70
...
ARRIVAL_RATES = [1.0, 2.0, 4.0, 6.0, 8.0]


def discover_scenarios(base_dir):
    """Scan ./request/ and ./resource/ folders next to this script.
