...
# Baseline window: seconds from the start of each resource file, when the system is empty
# (no requests, no pods) — captures node/monitoring overhead to subtract from usage.
BASELINE_WINDOW_S = 20.0

# Regex to parse filenames.
# `idle` is the idle-pool size; it can be a float (e.g. 0.0, 0.5) or an integer (e.g. 32).
FILE_RE = re.compile(
...
def arrival_dir(arrival_rate):
    ...
    # On-disk folders are e.g. "arrival_0.5" / "arrival_1.0" — use the str() form directly.
...
def discover_files(request_dir, resource_dir):
    """Scan directories and group files by (pod, idle) -> list of (rep, req_path, res_path)."""
...
def load_resource(path):
    ...
    with open(path) as f:
        for r in csv.DictReader(f):
            ...
            # "serving" = containers handling a request, i.e. both processing and starting
            # (cold-starting) ones — this replaces the old `serving_requests` column.
...
def compute_baseline(resource):
    """Baseline resource usage = mean over the first BASELINE_WINDOW_S of the resource file."""
    if not resource:
        return 0.0, 0.0, 0.0
    t0 = resource[0]["time"]
...
def analyze_rep(req_path, res_path, lower_cutoff=LOWER_CUTOFF, upper_cutoff=UPPER_CUTOFF):
    ...
    # Effective arrival rate: successfully served requests per second over the stable window
    # (i.e. admitted load, excluding blocked requests). This is the per-request normalizer.
    window_duration = t_end - t_start
    effective_arrival_rate = (len(successful_window) / window_duration
                              if window_duration > 0 else float("nan"))

    # Baseline is the mean over the first BASELINE_WINDOW_S of this rep's resource file.
    baseline_cpu, baseline_ram_pct, baseline_ram_mb = compute_baseline(resource)
    ...
    # Resource cost per request = resource above baseline divided by the effective arrival
    # rate, giving resource-seconds per request (e.g. %·s/req, MB·s/req).
    if res_window and effective_arrival_rate > 0:
        cpu_per_req = (avg_cpu - baseline_cpu) / effective_arrival_rate
        ram_pct_per_req = (avg_ram_pct - baseline_ram_pct) / effective_arrival_rate
        ram_mb_per_req = (avg_ram_mb - baseline_ram_mb) / effective_arrival_rate
...
def analyze_arrival(arrival_rate, lower_cutoff=LOWER_CUTOFF, upper_cutoff=UPPER_CUTOFF):
    ...
    for (pod, idle), reps in sorted(groups.items()):
        ...
        eff_arrival_rates = [r["effective_arrival_rate"] for r in rep_results]
        ...
        output_lines.append(f"══ Pod: {pod}, Arrival: {arrival_rate}, Idle: {idle} ({len(rep_results)} reps) ══")
        ...
        output_lines.append(f"  Eff. arrival (r/s):  mean={np.mean(eff_arrival_rates):.4f}  std={np.std(eff_arrival_rates):.4f}")
        output_lines.append(f"  CPU/req     (%·s) :  mean={np.mean(cpu_per_reqs):.4f}  std={np.std(cpu_per_reqs):.4f}")
        output_lines.append(f"  RAM/req     (%·s) :  mean={np.mean(ram_pct_per_reqs):.4f}  std={np.std(ram_pct_per_reqs):.4f}")
        output_lines.append(f"  RAM/req    (MB·s) :  mean={np.mean(ram_mb_per_reqs):.4f}  std={np.std(ram_mb_per_reqs):.4f}")
