...
Per-minute log fields (one row per (repetition, minute)):
  ...
  - cold_hit         : reactive cold starts (excludes pool-resize spawns)
  - redundant        : warm-pool pods that stayed Idle the entire minute
...
TRACE_NAME = "non_station.csv"

_TRACE_CONFIG = {
    "non_station.csv": {
        "model": "best_model_active_non_station",
        "log_subdir": "non_station",
    },
    "day_night.csv": {
        "model": "best_model_active_day_night",
...
def _build_quantile_lstm(n_features: int):
    """Architecture must match train_predictors_num_active.get_lstm_model()."""
...
def make_predictor(model_dir: Path):
    """Load the p99_active predictor.

    Returns (predict, lookback, model_type, feature_names) where ``predict``
    takes a 2-D array of shape (lookback, n_features) whose columns are
    ordered as ``feature_names`` (i.e. ['minute', 'arrival', 'p99_active'])
    and returns the predicted next-minute p99_active.
    """
    ...
    if bundle["model_type"] == "LSTM":
        ...
        weights_path = model_dir / bundle.get("lstm_path", "lstm.pth")
        model = _build_quantile_lstm(n_features).to(device)
    ...
    return predict, lookback, bundle["model_type"], feature_names
...
class MLSystem(System):
    ...
    def __init__(self, env, config, distribution, trace_per_minute,
                 use_ml, predictor, lookback, feature_names,
                 repetition, seed, verbose=False):
        ...
        # Column order expected by the predictor (e.g. ['minute', 'arrival', 'p99_active']).
        ...
        # Rolling per-minute feature history fed to the predictor.
        # Each entry mirrors `self.feature_names` order.
    ...
    def minute_ticker(self):
        ...
        for m in range(self.n_minutes):
            ...
            if self.use_ml:
                ...
                # Window: last `lookback` rows of (minute, arrival, p99_active);
                # model output IS the predicted next-minute p99_active, used
                # directly as the warm-pool target (quantile=0.7 already biases
                # upward, so no extra headroom is applied).
                if m >= self.lookback and len(self.feature_history) >= self.lookback:
                    window = np.asarray(
                        self.feature_history[-self.lookback:], dtype=float
                    )
                    predicted = max(0.0, float(self.predictor(window)))
                    next_target = math.ceil(predicted)
                    ...
                    self.pool_history.append({
                        "minute": m + 1,
                        "predicted_p99_active": predicted,
            ...
            self.minute_cold_start_count = 0
            self.minute_blocked_count = 0
            idle_snapshot_ids = {id(c) for c in self.idle_containers if c.state == "Idle"}
            ...
            fully_idle = sum(
                ...
                if id(c) in idle_snapshot_ids
                and c.state == "Idle"
                and c.idle_since <= minute_start
            )
            ...
            # Record this minute's features for the predictor window. Column
            # order must match `self.feature_names` (e.g. ['minute', 'arrival',
            # 'p99_active']). Trim to lookback length to bound memory.
            if self.feature_names:
                ...
                for col in self.feature_names:
                    if col == "minute":
                        ...
                        row.append(float(self.minute_arrivals_count))
                    elif col == "p99_active":
                        row.append(float(p99_active))
                    elif col == "p95_active":
                        row.append(float(p95_active))
                    else:
                        raise KeyError(f"Unknown feature column: {col}")
                self.feature_history.append(row)
                if self.lookback and len(self.feature_history) > self.lookback + 4:
                    # Keep a small slack beyond lookback to avoid per-step trimming churn.
                    del self.feature_history[:-self.lookback]
...
def run_one_repetition(rep_idx: int, seed: int, cfg: dict, trace: np.ndarray,
                       predictor, lookback: int, feature_names: list):
...
def print_run_summary(system: MLSystem):
...
def write_combined_log(all_rows, n_reps: int) -> Path:
    ...
    csv_path = LOG_DIR / f"{ts}_multi_x{n_reps}_mine.csv"
    fields = [
...
def main():
    ...
    feature_names: list = []
    if USE_ML:
        predictor, lookback, model_type, feature_names = make_predictor(MODEL_DIR)
    else:
        try:
    ...
    print(f"  Features:         {feature_names or '(none)'}")
    ...
    for rep in range(NUM_REPETITIONS):
        seed = BASE_SEED + rep
        system = run_one_repetition(rep, seed, cfg, trace, predictor, lookback, feature_names)
