...
class Container:
    """Represents a container instance running on a server."""
    ...
    def release_request(self):
        """Release the finished request and return the container to idle."""
        if not self.current_request:
            return

        self.system.update_resource_stats()
        # Flush warm/idle RAM before this pod flips back to Idle below; if it
        # stays in the warm pool it begins contributing idle RAM from now on.
        self.system.update_idle_ram_stats()
        ...
        # Cold-start containers are evicted immediately after serving one request.
        # Warm-pool containers (present in system.idle_containers) stay alive.
